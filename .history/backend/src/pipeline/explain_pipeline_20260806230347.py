...
class ExplainPipeline:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.gradcam.model = self.predictor.load_model()

            logging.info("Shared EfficientNet model with GradCAM")
    ...
    def explain(self, image_path):
        ...
        if (
            prediction_result["prediction"] == "PNEUMONIA"
            and self.gradcam is not None
        ):
            try:
                heatmap_path = self.gradcam.generate_heatmap(image_path)
            except Exception:
                logging.exception("GradCAM failed")
